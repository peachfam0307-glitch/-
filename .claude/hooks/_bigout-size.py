...
def main():
    ...
    # 🖼 [2026-09-10] 그림을 «바이트»로 세지 않는다 — 창업자 「왜 터지지? 33%밖에 안되는데」
    #    까닭 = 511×360 그림 한 장이 241,274 B 인데 실제로 먹는 건 ~1,500 토큰뿐이다.
    #    그래서 창이 33% 밖에 안 찼는데도 문턱(60,000 B)이 터졌다 — «과잉 경보»였다.
    #    ⛔ 문턱은 «안» 올린다(그건 땜빵이다 · 절대원칙 34). 세는 «자»를 고친다.
    #    ⚠️ 이 함수는 진작 있었는데 «부르는 줄»이 빠져 한 번도 안 돌았다 — 여기서 붙인다.
    _장수, 본문 = 그림빼기(본문)
