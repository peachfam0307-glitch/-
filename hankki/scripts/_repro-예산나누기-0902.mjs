...
console.log('④ 앱이 「남은 예산」을 실어 보낸다')
{
  ...
  try {
    ...
    // ⛔ 앱이 «실제로 기다리는 시간»보다 클 수 없다 — 크면 워커가 「아직 여유 있다」고 잘못 판단한다.
    // ⭐⭐ **[2026-09-08] 그 시간이 «늘었다» — 60초 → 2분.** 잣대가 낡아서 이 판이 배포를 막았다.
    //    ⛔ 「숫자를 늘린 땜빵」이 아니다(절대원칙 34) — 구조가 바뀌어 앱이 진짜로 그만큼 기다린다:
    //       워커가 번호표를 즉시 주고(202) 뒤에서 일하고, 앱은 선반을 «5초마다 24번»(=120초) 물어본다.
    //       그 값이 `tidy.js` 의 `선반간격 × 선반횟수` 이고, 여기 잣대는 그 «같은 사실»을 재는 것이다.
    //    📌 저 둘을 또 바꾸면 이 숫자도 같이 바꾼다(같은 잣대는 같은 말이라야 한다).
    const 앱이기다리는시간 = 5000 * 24
    칸('앱이 기다리는 시간을 안 넘는다', b.budgetMs <= 앱이기다리는시간, b.budgetMs)
