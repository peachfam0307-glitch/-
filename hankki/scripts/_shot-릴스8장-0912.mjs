...
const 첫편 = page.locator('.rc-card, .recipe-card, article').first()
if (await 첫편.count()) {
  await 첫편.click({ force: true }); await page.waitForTimeout(1600)
  await 찍기('5a-상세-위')
  await page.mouse.wheel(0, 900); await page.waitForTimeout(800); await 찍기('5b-상세-재료')
}
