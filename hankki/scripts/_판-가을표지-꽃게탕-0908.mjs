...
// 🖼🖼 **창업자가 «직접 꾸민» 표지의 짜임을 그대로 옮긴다** (2026-09-08 20:01 캡처 = 간장비빔국수)
//    📮 창업자 = *"이걸 꽃게탕으로"* ＋ *"우리 원형 가을 프레임 하나 더 넣자"*
//    🔢 좌표를 지어내지 않았다 — `docs/_내레시피-백업/2026-09-07.json` 의 그 편에서 «읽어 온» 값이다.
//    ⭐ 셋의 차이는 **원형 가을 프레임 하나뿐** — pf_au01 · pf_au03 · pf_au04
//       (셋 다 2026-09-01 에 열린 무료 가을 프레임 · `deco_autumn_frame`)
const 시안 = ['pf_au01', 'pf_au03', 'pf_au04'].map((프레임, i) => ({
  이름: `${'ㄱㄴㄷ'[i]} ${프레임}`,
  decorBg: 'none',
  thumb: 'icon',        // ⭐ 창업자 판과 같게 — 음식이 프레임 «안»에 보인다
  decor: [
    { id: 'f-frame', type: 'sticker', key: 프레임, x: 0.508, y: 0.53, s: 0.7, r: 0 },
    { id: 'f-title', type: 'note', text: '꽃게탕♡', font: 'gaegu', x: 0.481, y: 0.157, s: 0.62, r: -7.4, w: 'thin' },
...
const state = {
  ...
  recipes: 시안.map((v, i) => ({
    ...
    thumb: v.thumb || 'none',   // ⚠️ 창업자 판은 'icon' — 음식 그림이 프레임 안에 든다
