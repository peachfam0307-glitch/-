...
// ⛔⛔ [2026-09-08] 옛 판 = `p.mall || (p.url ? '직접' : '네이버')` — **url 이 붙은 제품을 전부 한 덩어리**로 묶었다.
//   🔢 그래서 링크를 달수록 「몰 1가지」로 «뭉쳐 보이는» 거꾸로 된 일이 났다
//      (오늘 링크 다섯을 채우자 뭉친 주가 1 → 2 로 늘었다. 실제로는 네이버·쿠팡·자사몰로 «갈라졌는데»도.)
//   ⭐ 이 검사가 재려는 건 「유저가 여러 곳에서 살 수 있나」다 → **유저 눈에 뜨는 구매처**로 세야 맞다.
//      그래서 `curation.js` 의 `productMall` 과 «같은 갈래»로 나눈다(글자 파싱이라 import 는 못 한다).
const 몰이름 = (p) => p.mall
  || (p.url
    ? (/coupang\./.test(p.url) ? '쿠팡'
      : /smartstore\.naver|brand\.naver/.test(p.url) ? '네이버'
        : (p.brand || '자사몰'))
    : '네이버')
const 주갈래 = []
for (let i = 0; i < 26; i++) {
