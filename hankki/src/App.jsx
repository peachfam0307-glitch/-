...
export default function App() {
  ...
  // 🖼🖼 **「뭘 많이 썼나」를 재는 자리** (2026-09-08)
  //
  // 📮 창업자 = *"이걸 지금부터 재야 향후 기능추가나 업뎃할때 이용하지"*
  //
  // ⭐⭐ **여기 «한 곳»이면 네 갈래가 다 잡힌다** — `push`·`replace`·`pop`·탭이동.
  //    부르는 자리마다 한 줄씩 심으면 새 화면을 만들 때마다 빠뜨린다(그러면 숫자가 «조용히» 샌다).
  //    보고 있는 화면은 언제나 「스택 맨 위, 없으면 지금 탭」이니 그 둘만 지켜보면 된다.
  //
  // ⛔ 나가는 건 «화면 이름 하나»뿐이다 — 레시피 제목·일기 글·검색어는 안 나간다.
  //    이름표를 거르는 자물쇠는 `src/stats.js` 의 `보내도되는화면` 이 갖는다.
  // ⛔ 통계를 끈 사람에겐 아무 일도 안 일어난다(`화면봄` 이 첫 줄에서 되돌아간다).
  useEffect(() => {
    const 위 = stack.length ? stack[stack.length - 1] : null
    화면봄(위 ? 위.name : tab)
  }, [tab, stack])
  ...
  useEffect(() => {
    ...
    consumeSharedIntake().then(async (data) => {
      ...
      if (data.imageDataUrl) {
        ...
        // 🤖 `AI끝남` — **AI 가 성공해서 그 결과로 채우는 판인가.** 졸업(status)만 이 값을 본다.
        //    ⛔ 기본값 `false` 다 — 부르는 쪽이 «말하지 않으면 졸업 안 시킨다». 새 자리가 생겨도 안전한 쪽으로 넘어진다.
        const 채우기 = (r, { AI끝남 = false } = {}) => {
          ...
          store.updateRecipe(rec.id, {
            ...
            //
            // 🗃🤖🤖 **[2026-09-05 · 창업자 확정] 졸업은 «AI 가 성공했을 때만».**
            //   📮 창업자 제보 = *"ai가 작동을안하는데도 **보관함에서 금방사라져버려. 읽기시작하면 바로 옮겨지는 듯**"*
            //   📮 창업자 확정 = *"보관함은 **ai가 읽어서 성공했을때만 옮기고**,
            //                    ai가 실패해서 **기본규칙이 적용되면 그대로 남게** 하자."*
            //   ⛔⛔ 뿌리 = 이 함수는 **두 판**으로 불리는데 졸업 잣대가 «둘 다 같았다» —
            //      ①규칙 파서 판(`채우기(기본)`)이 **AI 가 시작하기도 «전»에** 졸업을 박았다.
            //      🔢 실물(9/05 아침 인스타 원팬 파스타) = 제목 「계속됩니다.. 커밍쑨」(앞 게시물 꼬리) ·
            //         재료에 「바닥 5중, 옆면 3중에…」(팬 광고문). 그게 그대로 «정식 레시피»가 됐다.
            //      ＝ `다읽었나` 는 **재료>=2 && 걸음>=2** 가 전부라 **장사말 2줄이면 졸업한다.**
            //   ⭐⭐ 그래서 잣대를 «둘로 가른다» — 이게 이번 고침의 전부다:
            //      · **어디로 데려가나**(화면) = `다읽었나(parsed)` — ⛔**지금 그대로 안 건드린다**
            //      · **어디에 두나**(status)  = `다읽었나` **＋ AI 가 끝났나**
            //   ⛔⛔ **이건 「가두는 것」이 아니다** — 창업자가 2026-09-05 에 직접 바로잡았다:
            //      *"나는 **무조건 내보내라는 뜻이 아니었어**. 그때는 아예 보관함에서 안 나가고
            //        최근저장에 반영이 안 되니까 물어본 거야."*
            //      ＝ 2026-09-01 요구의 진짜 조건은 속도가 아니라 **「영영 안 나오는 상태를 만들지 마라」**.
            //      ✅ 탈출구 둘이 «살아 있다» — ⑴AI 성공하면 이 줄이 저절로 졸업시킨다
            //         ⑵실패해도 유저가 그 레시피를 열면 `RecipeDetailScreen` 이 만회하고 «그때» 졸업시킨다
            //         ⑶그래도 싫으면 임시보관함의 「그대로 저장」(InboxScreen) — 유저 손으로 언제든.
            //      🔒 그 셋이 살아 있는지 `_repro-보관함AI대기-0905.mjs` 가 잰다. ⛔판이 죽으면 배포가 막힌다.
            //   ⛔ **올리기만 한다 — 절대 내리지 않는다**(위 줄과 같은 원칙). 파서 판은 «안 올릴 뿐»
            //      이미 올라간 것을 내리지 않는다. 그래서 두 판의 차례가 바뀌어도 안 깎인다.
            ...(AI끝남 && 다읽었나(r) ? { status: 'sorted' } : {}),
          })
          현재.title = 새제목
        ...
        // ① 규칙 파서 — «즉시»
        // ⛔ 여기서는 **졸업시키지 않는다**(`AI끝남` 안 넘긴다) — 창업자 확정 2026-09-05.
        //    화면은 채워지고, 있을 자리만 「임시보관함」이다. AI 가 성공하면 ②가 저절로 꺼내 준다.
        채우기(기본)
