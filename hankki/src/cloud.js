...
export async function 올리기(백업, { 진행, 통째로, 기기, 안바뀌면건너뛰기 } = {}) {
  ...
  let 읽기수 = 0     // 🔢 이 올리기가 «읽은» 건수 — 부르는 쪽(저절로받기)이 계기판에 더한다
  const 새지문 = {}
  const 할일 = []
  const 건너뛴것 = []
  ...
  if (훑을까) {
    for (const 갈래 of ['recipes', 'diary']) {
      ...
      // 🔢 [2026-09-04] «읽은 만큼» 센다 — 계기판이 틀리면 감보다 나쁘다.
      //    ⛔ 이 훑기는 지문이 없을 때만 돈다(새 폰·앱 지웠다 깜·받은 직후 되올리기).
      읽기수 += (묶.size ?? (묶.docs || []).length)
      묶.forEach((문서) => {
  ...
  // ⛔⛔ **[2026-09-04 · 창업자 패드 실물이 잡았다] 도장을 «여기»에서 찍는다 — 부르는 쪽이 아니라.**
  //   📮 창업자 패드 화면 = *"가져온적있음 · 마지막으로 올린기기 «없음» · 09:02"*
  //      → 09:02 에 뭔가 올라갔는데 **도장이 비어 있었다.**
  //   🔎 왜 = 도장을 «부르는 쪽»이 넘겨야 했는데(`if (기기)`), 넘기는 건 `저절로올리기` 하나뿐이었다.
  //      **수동 「이 폰 것으로 덮기」와 로그인 첫 올리기는 안 넘겼다** → 도장 없이 올라갔다.
  //      그리고 받는 쪽은 `!d.기기` 를 「내가 마지막」으로 쳐서 **아무도 못 받는다.**
  ...
  if (안바뀌면건너뛰기 && !할일.length && 옛지문.meta === 새지문.meta) {
    지문쓰기(새지문)
    return { 건너뜀: true, 올린것: 0, 지운것: 0, 건너뛴것, 언제: '', 전부인가: true, 읽기: 읽기수 }
  }
...
/**
 * ⬆️ 저절로 올리기.
 * @param 확인함 ⭐**방금 «받기»를 돌려 클라우드를 확인했다**는 뜻. 그때는 안전장치 ①을 지난다.
 *   ⛔⛔ [2026-09-04] 안전장치 ①(마지막에 올린 게 내 기기가 아니면 안 올린다)은
 *      «덮어쓰기가 무서워서» 있는 것이다. 그런데 **받아서 합친 «직후»엔 덮을 게 없다** —
 *      내 손에 든 판이 이미 양쪽을 다 가졌기 때문이다.
 *      그 전엔 이 장치 때문에 **받은 기기가 영영 자기 것을 못 올렸다**(창업자 = 패드→폰 «감감무소식»).
 *   ⛔ 확인 «안» 했으면 그대로 막는다 — 인터넷이 끊겨 못 받았는데 올리면 그게 덮어쓰기다.
 */
...
export async function 저절로받기(백업만들기) {
  try {
    ...
    // ⭐⭐ **[2026-09-04] 「도장이 없다」를 «내가 마지막»으로 치지 않는다.**
    //   ⛔ 그 전엔 `!d.기기` 도 여기서 물러났다 — 그래서 **도장 없이 올라간 판은 아무도 못 받았다**
    //      (창업자 패드가 딱 그 상태였다 · 09:02 · 마지막 올린 기기 = 없음).
    //   ⭐ 「없다」는 «내가 올렸다»가 아니라 **«모른다»**다. 둘을 같게 다루면 안 된다.
    //      받기는 이제 «합치기»라 모르는 채로 받아도 **잃을 게 없다** → 모르면 «받아서 합친다».
    //   ⚠️ 값 = 읽기 한 번 더. 그리고 한 번 받고 나면 위 `올리기` 가 도장을 찍어 두 번 다시 안 온다.
    ...
    // ⬆️⬆️ **[2026-09-04 · 창업자 실물이 잡았다] 받아서 합쳤으면 «이어서 올린다».**
    //   📮 창업자 = 폰→패드는 왔는데 패드→폰은 *"아직 감감무소식.."*
    //   ⛔⛔ 왜 = 안전장치 ①(`cloud.js` 위쪽)이 「마지막에 올린 게 내 기기가 아니면 안 올린다」라
    //      **받은 기기는 도장을 못 받아 영영 자기 것을 못 올린다.** 물이 한 방향으로만 흘렀다.
    //   ⭐⭐ 그 안전장치를 «푸는» 게 아니다 — 그건 «덮어쓰기가 무서워서» 있는 것인데,
    //      **지금 손에 든 `판` 은 양쪽을 합친 것**이라 덮어도 잃을 게 없다.
    //      그래서 조건을 «합친 «직후»»로 못 박아 그 자리에서만 올린다(절대원칙 34 — 모양을 바꾼다).
    //   ⭐ 올리고 나면 도장이 이 기기로 넘어가 → **다음엔 상대가 받는다.** 그렇게 주고받는 모양이 된다.
    //   ⛔ 올리는 건 «방금 얹은 판»이다 — 옛 백업을 올리면 방금 받은 것이 도로 지워진다.
    //   ⚠️ 실패해도 «되물리지 않는다» — 받기는 이미 끝났고 되돌릴 벌도 있다.
    //      못 올리면 다음에 앱을 켤 때 다시 올라간다(그때는 안전장치 ①이 이 기기를 막지 않는다).
    //   📊 값 = 왕복 한 번마다 쓰기 한 묶음. 창업자 기기 둘이면 하루 6회쯤이다.
    //      ⛔ 전체 공개 전에 `syncPull`(바뀐 것만)을 붙일 것 — 1만 명이면 하루 3만 쓰기로 무료치를 넘는다.
    // ⛔⛔ **[2026-09-04 · 여기서 되올리지 «않는다»]**
    //   앞 판(v12.46)은 여기서 바로 올렸다. 그런데 그러면 «판을 얹기 «전»에» 네트워크를 한 번 더 타서
    //   그 사이에 유저가 담은 것이 얹기에 지워진다(창업자 = *"보관함에서 금방 사라져버려"*).
    //   ✅ 올리는 일은 **얹은 «뒤»에** 부르는 쪽이 한다(App.jsx) — 순서가 「받기 → 얹기 → 올리기」다.

    // 🔑🔑 **[2026-09-04 · 오늘의 뿌리] 지문은 «클라우드가 가진 것»이다 — «내가 가진 것»이 아니다.**
    //   ⛔⛔ 그 전엔 「지문맞추기(합친 판)」이었다. 합친 판엔 **클라우드에 없는 내 것도 들어 있다.**
    //      그걸 「올렸다」로 적으니 다음 올리기가 «바뀐 것 없음»으로 건너뛰어
    //      **폰 262 / 클라우드 260 이 «영원히» 안 맞았다**(창업자 실물 2026-09-04).
    ...
    try {
      ...
      // ⭐ «되올린 것»도 같이 적는다 — 그래야 화면에서 «주고받았다»가 읽힌다
      await L.적기({ 한일: '받음', 기기: d.기기 || '', 받은것: 받은편수, 합친것: 결과.레시피, 올린것: 되올림 })
