...
export default function PantryView() {
  ...
  const 급한줄제목 = (() => {
    const 담김 = []
    for (const n of 급한재료) {
      const 길이 = 담김.reduce((a, s) => a + s.length + 1, 0) + n.length
      if (길이 > 이름예산) break
      담김.push(n)
    }
    return 담김.length ? `${담김.join('·')}부터 쓰세요` : '빨리 먹어야 해요'
  })()
    ...
    <div className="hscroll inset" ref={칸} style={{ marginBottom: 4 }}>
      {목록.map(({ r, n, 급함표 }) => (
        <button key={r.id} className="grid-card press" style={{ flex: '0 0 41%', textAlign: 'left' }} onClick={() => nav.push({ name: 'detail', id: r.id })}>
          ...
          <div className="date">{급함표?.length ? `${expiryChip(급함표[0].남은날).text} ${급함표[0].이름} · 재료 ${n}개` : `가진 재료 ${n}개`}</div>
        </button>
      ))}
    </div>
  )
