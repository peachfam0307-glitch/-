...
export const WEEKLY = [
  ...
  },
  {
    ...
    // 🦀 [창업자 2026-09-07 00:05] *"간장게장보다 꽃게조림을 메인으로 올리자. 3개씩 하기로 했는데 2개로 줄이면서 꽃게조림 사라졌어"*
    //    홈은 앞 2편만 보인다(HomeScreen slice(0,2)) → 순서가 곧 «메인»이다. 조림·탕이 홈에, 간장게장은 레시피 탭에 그대로.
    why: '가을 꽃게는 암게라 알이 꽉 차요. 조리고 끓이고 담그는 세 가지로.',
  ...
  },
  {
    ...
    ids: ['basic-gaji-deopbap', 'basic-gaji-bokkeum'],
  },
  ...
  //
  // ✅✅ [2026-08-18] 빈 다섯 주를 «창업자 레시피»로 채웠다 — 아래가 그것이다.
  //    📮 창업자 = *"제철에 빈자리 있지 내 레시피중에 적당한거 골라봐"*
  //    ⭐ 52주 표(44 배추 · 45 무 · 46 가리비)를 그대로 쓰지 «않았다» —
  //       창업자 레시피 103편에서 **한 재료로 3편이 묶이는 것**을 골랐다.
  //       표는 「우리가 쓸 게 없을 때의 바닥」이고, 창업자 레시피가 있으면 그게 이긴다
  //       (주간 레시피 설계 = *"창업자 레시피가 있으면 그 주를 덮어쓴다"*).
  //    ⛔ 44 배추(김장)·45 무·46 가리비는 «안 쓴 채로 남는다» — 내년에 그대로 돈다.
  ...
  },
  {
    ...
    //    📮 창업자 = *"그거 내가 해먹는건데 적으면 돼."*
    //    ⛔ 백업 185편에도 앱에도 없어서 내가 지어내지 않는다. 받으면 여기 id 를 더한다.
    from: '2026-11-16', title: '전 부치는 날', kicker: KICKER_SPECIAL,
    why: '쌀쌀해지면 전이 당겨요. 재우지 않고 바로 부치는 것들로 골랐어요.',
    ids: ['basic-chogandan-yukjeon', 'basic-ojingeo-gamjajeon'],
  },
  {
    // 🍢 창업자가 «아껴둔» 자기 레시피 둘(2026-08-14 *"어묵탕 어묵볶음 좋다."*).
    //    ⭐ 둘은 «한 짝»이다 — 어묵탕 재료가 「전처리한 어묵(어묵볶음 편 참고)」을 부른다.
    //    ⛔ 이 줄은 원래 HOMEMADE 에 잘못 들어가 있었다 → 그 자리 주석 참조.
  ...
  //    ⚠️ 51 묵은지는 11/30 에 «먼저 써버렸다» — 12월엔 다른 것으로.
  //    ⭐ 창업자 레시피가 아직 89편 남아 있다(103 − 이번 14편).
]
...
export const HOMEMADE = [
  ...
  // 🍢🍢 [2026-08-18 고침] 11/23 「어묵」 줄을 **WEEKLY(제철)로 옮겼다** — 여기 있으면 안 된다.
  //    ⛔⛔ 두 줄이 «같은 배열»에 같은 날짜로 있으면 `열린줄()` 이 **하나만** 고른다
  //       (`w.from > pick.from` 이라 같은 날짜면 먼저 나온 것이 이긴다).
  //       → 11/23 에 「어묵」이 «우리집레시피 자리»를 먹고 **부대찌개·닭볶음탕이 영영 안 떴다.**
  //    🔬 코드로 재현해서 확인했다(2026-08-18) — 「우리집 어묵 — 어묵탕, 어묵볶음」이 찍혔다.
  //    📌 이건 2026-08-14 에 어묵을 넣을 때 «배열을 잘못 고른» 것이다. 레시피는 멀쩡했다.
...
// 🚨🚨 **[2026-09-09 창업자 제보] 「sns한끼 안열렸어 수요일오픈아냐?」 — 여기가 범인이었다.**
//   🔎 레시피는 «제대로 열려 있었다**(명란 바지락 파스타·애호박 국밥 · from 2026-09-09 · 실측).
//      그런데 홈 상자엔 **9/3 에 고른 두 편이 그대로** 서 있었다 — 창업자 폰 화면으로 확인.
//   ⛔⛔ 뿌리 = 위 주석이 *"큐레이션이라 목록 관리와 다르다"* 고 적었지만, **결과가 같았다.**
//      「그 주에 고른 것」이 «항상 그것»이 되어 **새로 열린 편을 영영 가린다.** 손으로 박은 목록은 반드시 낡는다(절대원칙 34).
//   ✅ 📮 창업자 = *"자동"* → **비운다.** 아래 `snsNow` 가 «최근에 열린 순»으로 저절로 채운다 —
//      매주 수요일 새 편이 저절로 선다. ⛔여기에 다시 id 를 박지 말 것.
//   ⭐ 특별히 앞에 세우고 싶은 주가 있으면 그때만 창업자가 말한다 — 그건 «그 주만» 넣고 다음 주에 뺀다.
const SNS_PICK = []
...
export const snsNow = (recipes = [], now = new Date(), k = 2) => {
  ...
  return {
    kicker: 'SNS 요리',
    // ✅ [창업자 확정 2026-09-03] *"sns에서 데려온 한끼 좋다"* — ⛔이 글자를 내 마음대로 안 바꾼다
    //   ⚠️ 바로 앞에 *"영상 보며 만드는 한끼로 가자"* 도 왔다 → **뒤엣것으로 갔다.**
    //   ⭐ 뜻으로도 이쪽이 맞다 — 인스타 편은 앱에서 재생이 «안 되는» 편이라 「영상」이 아닌데
    //      같은 상자에 선다. 「SNS」는 매체를 안 가린다(칩 이름을 SNS 로 고른 것과 같은 이유).
    title: 'SNS에서 데려온 한끼',
    // ⛔ [창업자 2026-09-03] *"유튜브·인스타에서 보고 우리 말로 정리했어요.-이말은 빼"*
    //   → 앞문장을 뺐다. ⛔이 자리에 다시 넣지 말 것.
    why: '원본은 레시피 안에서 볼 수 있어요.',
    items: 것들,
  }
}
