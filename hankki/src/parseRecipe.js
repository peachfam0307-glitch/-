...
export function 머리에서제목 (raw) {
  ...
  for (const 원줄 of 줄들) {
    ...
    // 🍽🍽 **[2026-09-08 · 창업자 제보] 제목 «꼬리»에 붙은 인분은 떼고 본다.**
    //   📮 창업자 실물 = 「무화과 부라타 잠봉 샐러드 2인분」 → 제목이 통째로 버려져 **「사진 레시피」**가 됐다.
    //   ⛔⛔ 뿌리 = `QTY` 는 «줄 아무 데나» 걸리는 잣대다(`^`·`$` 가 없다).
    //      바로 아래 `QTY_ONLY`·`PORTION_ONLY` 는 «줄 전체»를 보는데 `QTY` 만 성격이 달랐고,
    //      그래서 **이름 뒤에 인분이 붙기만 하면 이름째 버렸다.** 인스타 레시피 제목의 흔한 모양이다.
    //   ⭐ 고침의 «모양» = 숫자를 넓히는 게 아니라 **「버린다」를 「떼고 본다」로 바꾼다**(절대원칙 34).
    //      떼고 남은 게 이름 값을 하면(한글 두 자↑) 그걸 쓰고, 없으면 그때 버린다(「2인분」 단독은 그대로 버려진다).
    //   ⛔ 꼬리에서만 뗀다 — 가운데·머리의 분량은 그대로 걸러야 한다(「돼지고기 300g」은 이름이 아니다).
    const 인분뗀 = s.replace(/\s*\d+\s*[~\-–]?\s*\d*\s*인\s*[분저본부]?\s*(기준)?\s*$/, '').trim()
    if (인분뗀.length >= 2 && /[가-힣]{2,}/.test(인분뗀)) s = 인분뗀
