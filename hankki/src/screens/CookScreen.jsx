...
export default function CookScreen({ id }) {
  ...
  const finish = () => {
    ...
    //   ⛔⛔ **「만들었어요」 문은 «두 곳»이다** — 상세의 단추(`RecipeDetailScreen.onCook`) ＋ 여기.
    //      한 곳만 붙이면 반쪽이 된다. v11.74·v11.77 의 「레꾸자랑 구멍 넷」이 정확히 그 모양이었다.
    //   ⭐ `nav.popAll()` 로 이 화면은 사라지지만 시트는 `App` 이 그린다 — 그래서 안 죽는다.
