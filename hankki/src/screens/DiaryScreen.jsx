      ...
      {open && (
        <DecorEditor
          ...
          onSave={(items) => {
            ...
            // 🚪 리뷰 문 — 「오늘 일기를 남겼다」 (창업자 확정 2026-09-03)
            //   ⭐ 왜 «꾸미기 저장»인가 = 일기 «본문»은 350ms 디바운스 자동저장이라
            //      「저장한 순간」이 없다. 타이핑 «중»에 시트가 뜨면 최악이다.
            //      꾸미기 저장은 유저가 «단추를 눌러» 끝내는 뚜렷한 완료다.
            //   ⛔ 비웠을 땐 안 청한다(그건 되돌린 것이다 · 레꾸와 같은 결)
            //   ⭐ 글을 «쓴» 사람도 잡는다 — 본문은 350ms 자동저장이라 그 자체로는 문이 없다.
            //      🔢 실측(`_probe-일기문-0903`) = 꾸미기 판은 «첫 탭이 속지 고르기»라
            //         스티커 칸이 0개다 → 스티커만 잣대로 삼으면 «글만 쓴 사람»이 통째로 샌다.
            //   ⏳ 남는 한계 = 글만 쓰고 «꾸미기 저장을 안 누르면» 이 문은 안 열린다.
            //      (넓히려면 「일기 화면을 나갈 때」로 옮긴다 — 다음 판)
            if (items.length || String(entry?.note || '').trim()) nav.askReviewSoon?.('일기')
