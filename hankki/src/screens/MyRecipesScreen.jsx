...
export default function MyRecipesScreen({ initView = 'grid' }) {
  ...
  // 🔖 [2026-09-08] «방금 종을 바꾼 편» — 걸린 목록에서 그 자리에 남겨 둔다(위 `list` 참조).
  //   ⛔ 저장값이 아니다. 화면에만 산다 — 탭을 떠났다 오면 원래 잣대대로 걸린다.
  ...
  const list = query
    ? sorted.filter(hit)
    : folder === '전체' ? sorted
      // 🚨🚨 **[2026-09-08 밤] 「눌렀더니 다 사라졌다」의 진짜 뿌리가 여기였다.**
      //   📮 창업자 = *"다 지워지잖아!!!!!!!!! 그래서 내가 제보했잖아"* ＋ *"하트누르니까 차례로 사라졌어"*
      //   🔎 재현(`_repro-핀순환-옛핀-0908.mjs`)으로 눈으로 봤다 — 순환은 «제대로» 돈다.
      //      문제는 **「해볼 것」을 걸어놓고 누르면 그 편이 최애로 옮겨가 «보던 목록에서 즉시 빠지는» 것**이다.
      //      → 유저 눈엔 「눌렀더니 없어졌다」로 보인다. 하나씩 누르면 하나씩 없어진다.
      //   ⭐⭐ 실패의 «모양»을 바꾼다(절대원칙 34) — 종을 바꾼 편은 **그 자리에 남긴다.**
      //      ⛔ 목록에서 빼지 않는다. 어디로 갔는지는 클립 그림(모자↔하트)이 말해준다.
      //      ✅ 그래서 「사라졌다」가 아니라 「바뀌었다」로 보인다. 잃는 것이 없다.
      //   ⚠️ 화면을 떠났다 오면 원래 잣대대로 걸린다(방금 표는 화면에만 산다) — 저장값은 안 건드린다.
                    ...
                    <div key={r.id} className="grid-card" style={{ position: 'relative' }}>
                      ...
                      {!edit && (
                        <button
                          ...
                          /* 🔁🔁 [2026-09-08 밤] 눌렀을 때 «무엇이 됐는지»를 말해준다.
                                📮 창업자 = *"하트누르니까 차례로 사라졌어"* — 조용히 바뀌니 사라진 걸로 보였다.
                                ⭐ 말해주기 ＋ 그 자리에 남기기(`방금바꾼`) 둘이 한 쌍이다. 하나만 하면 반쪽이다. */
                          onClick={(ev) => {
                            ev.stopPropagation()
                            const 다음 = nextPin(r)
                            setFavPin(r.id, 다음)
